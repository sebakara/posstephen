...
<section class="content">
    ...
    <div class="row">
        ...
        <div class="form-group">
            ...
            <select name="category_id" id="category_id" class="form-control select2" placeholder="{{ __('messages.please_select') }}">
                @foreach($categories as $categoryId => $categoryName)
                    <option value="{{ $categoryName->id }}" {{ !empty($duplicate_product->category_id) && $duplicate_product->category_id == $categoryId ? 'selected' : '' }}>
                        {{ $categoryName->name }}
                    </option>
                @endforeach
            </select>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        // fetch category data for rra saving   category_id
